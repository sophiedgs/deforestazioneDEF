...
import type { GeoJsonData, BoundsPayload, ItaliaMapProps,} from "./Utils";
...
function ItaliaMap({ ricercaMappa, ricercaParticella, resetMappaCount, resetParticellaCount, menuRicercaVisibile,
  ...
  const [particelleVeneto, setParticelleVeneto] =
  ...
  const normalizzaNomeFile = (path: unknown) => {
    const valore = String(path ?? "");
    const parti = valore.split("/");
    const nomeFile = parti.pop();

    if (nomeFile) {  return nomeFile;  }
    return "";
  };
  ...
  const getImageSrc = (path: unknown) => {
    ...
    for (const [key, value] of Object.entries(immaginiPunti)) {
      if (key.endsWith(`/${nomeFile}`)) { return value; }
    }

    return "";
  };

  const fotoGalleria = rilevazioniPunto.filter((rilevazione) => {
    const immagine = getImageSrc(rilevazione.image_path);

    if (immagine) { return true; }
    return false;
  });

  let fotoCorrente: Record<string, unknown> | null = null;

  if (fotoGalleria[indiceFotoGalleria]) {
    fotoCorrente = fotoGalleria[indiceFotoGalleria];
  }

  const evidenziaMappaStyle: L.PathOptions = {color: "#d81b60", weight: 1, fillColor: "#f06292",fillOpacity: 0.4,};
  const evidenziaParticellaStyle: L.PathOptions = { color: "#1b5e20", weight: 1, fillColor: "#2e7d32", fillOpacity: 0.35,  };
  ...
  const aggiornaDatiMappa = useCallback(async (bounds: BoundsPayload) => {
    ...
    try {
      const responseMappe = await fetch(`http://localhost:3001/api/mappe_veneto_official?${params}` );
      if (!responseMappe.ok) { throw new Error("Errore caricamento mappe catastali");   }
      const datiMappe: GeoJsonData = await responseMappe.json();

      const responseParticelle = await fetch( `http://localhost:3001/api/particelle_veneto_official?${params}`);
      if (!responseParticelle.ok) {throw new Error("Errore caricamento particelle catastali");}
      const datiParticelle: GeoJsonData = await responseParticelle.json();
  ...
  const analizzaPoligono = useCallback(
    async (feature: Feature, layer: L.Layer) => {
      try {
        ...
        const response = await fetch( "http://localhost:3001/api/analizza_poligono",{
            method: "POST", headers: { "Content-Type": "application/json" },  body: JSON.stringify({ geojson: feature }), }
        );
        ...
        if (!response.ok) {
          if (response.status === 400) {
            ...
            if (result.error) {
              setErrorePoligono(result.error);
            } else {
              setErrorePoligono("Ridisegna il poligono entro i confini del Veneto." );
            }
          ...
          throw new Error("Errore nell'analisi del poligono");
        }
        ...
        if (result.particelleIntersecate) {
          ...
          if (result.particelleIntersecate.features) {
            features = result.particelleIntersecate.features;
          }

          if (features.length > 0) {
            setSelezioneMappa({ origine: "poligono", tipo: "poligono", dati: result.riepilogoPoligono, });
          }
        }
      } catch (err) {
        console.error("Errore analizzaPoligono:", err);

        if (err instanceof Error) { setErrorePoligono(err.message); }
        else { setErrorePoligono("Errore nell'analisi del poligono."); }
      }
    },
    [resetDatiPunto]
  );
  ...
  const evidenziaFeatureRicercata = useCallback(
    (geojson: GeoJsonData | null, tipo: "mappa" | "particella") => {
      if (!mapRef.current || !geojson || !geojson.features || geojson.features.length === 0) { return; }
      ...
      if (tipo === "mappa") { setFeatureMappaRicercata(geojson);} 
      else { setFeatureParticellaRicercata(geojson); }

      const properties = (feature.properties ?? {}) as Record<string, unknown>;

      let tipoRisultato: "provincia" | "comune" | "particella" | "mappa";

      if (properties.tipo === "provincia") { tipoRisultato = "provincia";
      } else if (properties.tipo === "comune") { tipoRisultato = "comune";
      } else if (properties.tipo === "mappa") {  tipoRisultato = "mappa";
      } else if (properties.tipo === "particella") { tipoRisultato = "particella";
      } else {tipoRisultato = tipo; }

      setSelezioneMappa({ origine: tipo, tipo: tipoRisultato, dati: properties, });
  ...
  const resetSoloMappa = useCallback(() => {
    ...
    setSelezioneMappa((prev) => {
      if (prev && prev.origine === "mappa") { return null; }

      return prev;
    });

    if (!featureParticellaRicercata) {
      if (mapRef.current) {
        if (venetoBoundsRef.current) {
  ...
  const resetSoloParticella = useCallback(() => {
    ...
    setSelezioneMappa((prev) => {
      if (prev && prev.origine === "particella") {return null;}

      return prev;
    });

    if (!featureMappaRicercata) {
      if (mapRef.current) {
        if (venetoBoundsRef.current) {
          mapRef.current.fitBounds(venetoBoundsRef.current, {padding: [20, 20],});
        }
  ...
  useEffect(() => {
    ...
    const cercaParticella = async () => {
      ...
      if (ricercaParticella.provincia) {
          params.append("provincia", ricercaParticella.provincia);
      }

      if (ricercaParticella.comune) {
        params.append("comune", ricercaParticella.comune);
      }

      if (ricercaParticella.particella) {
        params.append("particella", ricercaParticella.particella);
      }
  ...
  const selezionaParticellaDaPopup = useCallback(
  ...
  const caricaDatiParticella = useCallback(async () => {
    let particellaId: unknown = null;

    if (selezioneMappa && selezioneMappa.dati.id) {
      particellaId = selezioneMappa.dati.id;
    } else if (selezioneMappa && selezioneMappa.dati.id_db) {
      particellaId = selezioneMappa.dati.id_db;
    } else if (selezioneMappa && selezioneMappa.dati.particella_id) {
      particellaId = selezioneMappa.dati.particella_id;
    }
    ...
    if (onChiudiMenuRicerca) {
      onChiudiMenuRicerca();
    }

    try {
      const response = await fetch( `http://localhost:3001/api/dati_particella/${
        encodeURIComponent( String(particellaId) )}` );
  ...
  const selezionaPunto = useCallback(
    async (punto: Record<string, unknown>) => {
      const puntoId = punto.punto_id;
      if (!puntoId) { return; }

      const response = await fetch(`http://localhost:3001/api/rilevazioni_punto/${encodeURIComponent(String(puntoId))}` );
  ...
  let puntiDaMostrare = puntiParticella;

  if (puntoSelezionato && puntiParticella) {
    puntiDaMostrare = {
      ...puntiParticella,
      features: puntiParticella.features.filter((feature) => {
        if (feature.properties?.punto_id === puntoSelezionato.punto_id) {
          return true;
        }
  ...
  let classeLayout = "layout-mappa-dettagli";

  if (selezioneMappa) {
    classeLayout += " layout-con-dettagli";
  } else {
    classeLayout += " layout-senza-dettagli";
  }

  if (!menuRicercaVisibile) {
    classeLayout += " layout-dettagli-esteso";
  }
        ...
        <div className="contenitore-mappa">
          {!error && (
            <MapContainer center={position} zoom={8} scrollWheelZoom={true} 
              style={{ height: "700px", width: "100%" }} ref={mapRef} >
              ...
              <LayersControl position="topright">
                ...
                <LayersControl.BaseLayer name="Mappa Satellitare">
                  <WMSTileLayer url="https://idt2-geoserver.regione.veneto.it/geoserver/ows"
                ...
                <LayersControl.Overlay checked name="Confini Veneto">
                  <LayerGroup>
                    {datiVeneto && (
                      <GeoJSON key={`veneto-${datiVeneto.features.length}`} data={datiVeneto} style={venetoStyle} />
                    )}
                  </LayerGroup>
                ...
                <LayersControl.Overlay checked name="Mappe Veneto">
                  <LayerGroup>
                    {mappeVeneto && mappeVeneto.features.length > 0 && (
                      <GeoJSON key={`mappe-${mappeVeneto.features.length}`} data={mappeVeneto}  style={mappaStyle}
                        onEachFeature={(feature, layer) => {
                          layer.on("click", () => {
                            ...
                            setSelezioneMappa({ origine: "mappa", tipo: "mappa",
                              dati: (feature.properties ??
                ...
                <LayersControl.Overlay checked name="Particelle Veneto">
                  <LayerGroup>
                    {particelleVeneto && particelleVeneto.features.length > 0 && (
                        <GeoJSON key={`particelle-${particelleVeneto.features.length}`} data={particelleVeneto} style={particelleStyle}
                          onEachFeature={(feature, layer) => {
                            layer.on("click", () => {
                              const pathLayer = layer as L.Path;
                              ...
                              setSelezioneMappa({ origine: "particella", tipo: "particella",
                                dati: (feature.properties ??
                                  {}) as Record<string, unknown>,
                              });
                            });
                ...
                {particelleDisegno && particelleDisegno.features.length > 0 && (
                  <GeoJSON key={`particelle-disegno-${JSON.stringify(
                      particelleDisegno.features.map(
                        (feature) => feature.properties?.id
                      )
                    )}`}
                    data={particelleDisegno} style={evidenziaParticellaStyle}
                    onEachFeature={(feature, layer) => {
                      layer.on("click", () => {
                        resetDatiPunto();
                ...
                {featureMappaRicercata && (
                  <GeoJSON key={`feature-mappa-${JSON.stringify(featureMappaRicercata.features[0]?.properties ?? {})}`}
                    data={featureMappaRicercata} style={evidenziaMappaStyle} />
                )}

                {featureParticellaRicercata && (
                  <GeoJSON key={`feature-particella-${JSON.stringify( featureParticellaRicercata.features[0]?.properties ?? {}                   )}`}
                    data={featureParticellaRicercata} style={evidenziaParticellaStyle}/>
                )}

                {puntiDaMostrare && puntiDaMostrare.features.length > 0 && (
                  <GeoJSON key={`punti-particella-${JSON.stringify(puntiParticella?.features.map(
                        (feature) => feature.properties?.punto_id
                      ) ?? []
                    )}-selected-${puntoSelezionato?.punto_id ?? "none"}`}
                    data={puntiDaMostrare}
                    pointToLayer={(feature, latlng) => {
                      const puntoId = feature.properties?.punto_id;
                      let isSelezionato = false;

                      if (puntoSelezionato) {
                        if (puntoSelezionato.punto_id === puntoId) {
                          isSelezionato = true;
                        }
                      }
                      ...
                      return L.circleMarker(latlng, {
                        radius, weight: 1, color, fillColor, fillOpacity: 0.9, });
                    }}
                    onEachFeature={(feature, layer) => {
                      layer.on("click", () => {
          ...
          <div className="pannello-dettagli-destro">
            <button type="button"
              onClick={() => {
                ...
                if (onApriMenuRicerca) {onApriMenuRicerca();}
              }}
            ...
            {selezioneMappa.tipo === "particella" && (
              <div>
                ...
                {typeof selezioneMappa.dati.numero_particelle_intersecate === "number" && (
                  <p><strong>Particelle intersecate:</strong>{" "}{String(selezioneMappa.dati.numero_particelle_intersecate)}</p>
                )}

                {!mostraDettagliParticella && (
                  <button type="button" className="cercaCancella_selezione" onClick={caricaDatiParticella}>
                    Visualizza dati
                  </button>
                )}
                      ...
                      <>
                        ...
                        <div className="elenco-particelle-orizzontale">
                          {puntiParticella?.features.map((feature, index) => {
                            ...
                            return (
                              <button key={`${String(props.punto_id)}-${index}`} type="button"
                      ...
                      <div className="pannello-rilevazioni-punto">
                        ...
                        <div className="testata-punto-galleria">
                          ...
                          <div className="galleria-punto">
                            {fotoCorrente && (
                              <>
                                <img src={getImageSrc(fotoCorrente.image_path)}
                                ...
                                <div className="galleria-controlli">
                                  ...
                                  <span> {indiceFotoGalleria + 1}/ {fotoGalleria.length} </span>

                                  <button  type="button"
                                    onClick={() => {
                                      setIndiceFotoGalleria((prev) => {
                                        if (prev === fotoGalleria.length - 1) {return 0;}
                            ...
                            {!fotoCorrente && ( <div className="galleria-vuota">Nessuna foto</div> )}
                          </div>
                        </div>
                        ...
                        {rilevazioniPunto.map((r, index) => (
                          <div key={String(r.rilevazione_id)} className="rilevazione-card" >
                            ...
                            <p><strong>Immagine:</strong>{" "}<button type="button" className="link-visualizza-foto"
            ...
            {selezioneMappa.tipo === "poligono" && (
              <div>
                ...
                <p><strong>N° particelle intersecate:</strong>{" "}
                  {String( selezioneMappa.dati.numero_particelle_intersecate ?? 0 )} </p>

                {Array.isArray(selezioneMappa.dati.elenco_particelle) && (
                  <div>
                    <div className="elenco-particelle-orizzontale">
                      {(selezioneMappa.dati.elenco_particelle as Array<Record<string, unknown>>)
                        .slice(0, 80).map((p, index) => (
                          <button key={`${String(p.nome_mappa)}-${String(p.nome_particella)}-${index}`}
                            type="button"  className="badge-particella badge-particella-btn"
